...
export default {
  ...
  computed: {
    ...mapGetters({ consumerStore: ConsumerStoreTypes.GETTER_CONSUMER_STORE }),
  },
  mounted() {
    this.consumerstore = this.consumerStore
  },
  methods: {
    ...mapMutations([
    ...
    ...mapActions([
      ConsumerStoreTypes.ACTION_EDIT_CONSUMER_STORE,
    ]),
    onSubmit() {
      this.spinner = true
      ...
      this.$refs.validateForm.validate().then(async success => {
        if (success) {
          ...
          const response = await this[ConsumerStoreTypes.ACTION_EDIT_CONSUMER_STORE]()
